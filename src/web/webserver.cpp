    ...
    _server->onNotFound([this](AsyncWebServerRequest *request) {
        ...
        if (isBodyPostEndpoint(path)) {
            ...
            if (path.indexOf("/api/scenes/") == 0) {
                ...
                else if (path.endsWith("/channels"))    _handleUnitChannels(request);
...
// Dual-dimmer fixtures (e.g. Oligo Grace: independent Uplight/Downlight
// channels, NOT a single vertical-balance value). Body: {"up":0-255,"down":0-255}
void CasambiWebServer::_handleUnitChannels(AsyncWebServerRequest* request) {
    uint8_t unitId;
    if (!_checkBle(request)) return;
    CasambiUnit* unit = _unitFromPath(request, "/channels", unitId);
    if (!unit) return;

    JsonDocument doc;
    uint8_t up, down;
    if (!_parseBody(request, doc) || !_requireUint8(request, doc, "up", up) ||
        !_requireUint8(request, doc, "down", down)) return;

    if (!_client->setUnitChannels(unitId, up, down)) { _sendBleSendError(request); return; }

    WEB_LOG("Web: Unit %d (%s) channels up=%d down=%d from %s\n", unitId, unit->name.c_str(), up, down, _getClientIP(request).c_str());
    _sendJsonSuccess(request);
}
