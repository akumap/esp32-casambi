...
| **E6c** | Sauberes Abmelden? | **`exit` / `quit`**, nur über Telnet, nicht in `help` (die teilt sich die serielle Konsole, dort gibt es nichts zu verlassen) | Folgt direkt aus E6: ohne Logout bleibt der einzige Slot belegt, bis der Idle-Timeout greift oder der Verbindungsabbruch auffällt — das blockiert die nächste Anmeldung von einem anderen Rechner. `plink` hat kein Telnet-Escape (`Strg+]` ist eine Eigenschaft des klassischen `telnet`-Clients, nicht des Protokolls), also ist `exit` der einzige In-Band-Ausstieg. |
